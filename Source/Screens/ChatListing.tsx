    ...
    const RenderItem = ({ item, index }: any) => {
        return (
            <Pressable onPress={() => {
                ...
                <View style={{ flex: 1 }}>
                    <FastImage source={{ uri: `${item?.business_picture}` }} style={{ width: wp(12), height: wp(12), borderRadius: 25 }} resizeMode="contain" />
                </View>

                <View style={{ flex: 7, marginLeft: wp(5) }}>
                    <Text style={{ fontSize: 18 }}>{item?.business_name}</Text>
