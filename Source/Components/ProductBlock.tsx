...
const ProductBlock = ({
    ...
    const toggleFollowInSubcollections = async () => {
        ...
        if (result == true) {
            statusChangingForFollow(blockItem?.user_id, true)
        } else if (result == false) {
            statusChangingForFollow(blockItem?.user_id, false)
        }
    }
    ...
    const toggleSavingCollection = async () => {
        ...
        if (result == true) {
            onSavePress(blockItem?.id, true)
        } else if (result == false) {
            onSavePress(blockItem?.id, false)
        }
    }
    ...
    const LikingProduct = async () => {
        if (blockItem?.isLiked == true) {
            setBlockItem({
                ...blockItem,
                isLiked: false,
                likeCount: blockItem?.likeCount - 1
            })
        } else if (blockItem?.isLiked == false) {
            setBlockItem({
                ...blockItem,
                isLiked: true,
                likeCount: blockItem?.likeCount + 1
    ...
    return (
        <>
            <View style={styles.mainView}>
                ...
                <View style={showShopName && { flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between' }}>
                    {showShopName &&
                        <Pressable onPress={() => {
                            ...
                            <View>
                                <FastImage source={{ uri: blockItem?.businessUser?.photo }} style={{ width: 50, height: 50, borderRadius: 25 }} resizeMode="contain" />
                            </View>
                            <View style={{ marginLeft: 10 }}>
                                <Text style={{ fontSize: 18 }}>{blockItem?.businessUser?.name}</Text>
                            </View>
                        </Pressable>
                    }

                    {showFollowButton && <View style={{ alignSelf: 'flex-end' }}>
                        {
                            blockItem?.follow
                                ?
                                <TouchableOpacity onPress={toggleFollowInSubcollections} style={{ borderWidth: 1, borderColor: 'black', padding: 5, paddingHorizontal: 20, borderRadius: 10 }}>
                ...
                <View style={{ width: '95%', }}>
                    {blockItem?.title && <Text style={{ fontSize: 18 }}>{blockItem?.title}</Text>}
                    {blockItem?.productType && <Text style={{ fontSize: 18, color: 'grey', fontWeight: 700 }}>{blockItem?.productType}</Text>}
                    {blockItem?.createdAt && <Text style={{ fontSize: 14 }}>{formatingDate(blockItem?.createdAt)}</Text>}
                </View>
                ...
                <View style={[styles.bottomView, {}]}>
                    <View style={styles.bottomSubview}>
                        ...
                        <View style={{ marginLeft: 4 }} >
                            <Text style={{ color: 'black', fontSize: 18 }}>{blockItem?.viewCount > 0 ? formatCount(blockItem?.viewCount) : 0}</Text>
                        </View>

                        <Pressable onPress={LikingProduct} style={{ marginLeft: 6 }}>
                            <Image
                        ...
                        <View style={{ marginLeft: 4 }} >
                            <Text style={{ color: 'black', fontSize: 18 }}>{blockItem?.likeCount > 0 ? formatCount(blockItem?.likeCount) : 0}</Text>
                        </View>
